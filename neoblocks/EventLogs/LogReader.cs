...
namespace Neo.Plugins
{
    public class LogReader : Plugin, IRpcPlugin, IPersistencePlugin
    {
        ...
        public override string Name => "EventLogs";
        ...
        {
            if (method != "geteventlog") return null;
        ...
        public void OnPersist(Snapshot snapshot, IReadOnlyList<Blockchain.ApplicationExecuted> applicationExecutedList)
        {
            ...
            foreach (var appExec in applicationExecutedList)
            {
                ...
                json["executions"] = appExec.ExecutionResults.Select(p =>
                {
                    ...
                    execution["events"] = p.Events.Select(q =>
                    {
                        JObject Event = new JObject();
                        Event["contract"] = q.ScriptHash.ToString();
                        Event["type"] = q.Type.ToString();
                        try
                        {
                            Event["state"] = q.State.ToParameter().ToJson();
                        }
                        catch (InvalidOperationException)
                        {
                            Event["state"] = "ERROR";
                        }
                        if (Event["type"].AsString() == "Contract.Create" || Event["type"].AsString() == "Contract.Migrate")
                        {
                            try
                            {
                                UInt160 script_hash = UInt160.Parse(q.ScriptHash.ToString());
                                ContractState contract = snapshot.Contracts.TryGet(script_hash);
                                Event["state"] = contract.ToJson();
                            }
                            catch (InvalidOperationException)
                            {
                                Event["state"] = "ERROR";
